...
import { Play, Pause, RotateCcw, Monitor, RefreshCw, Layers, Compass, Video } from 'lucide-react';
...
export default function ControllerDashboard() {
  ...
  useEffect(() => {
    ...
    setDriftHistory(prev => {
      ...
      if (next.length > 20) {
        next.shift(); // Keep only last 20 ticks
      }
      return next;
    });
  }, [displays]);
  ...
  const getStatusColor = (drift: number) => {
  ...
  const getStatusLabel = (drift: number) => {
    const absDrift = Math.abs(drift);
    if (absDrift < 150) return 'In Sync';
    if (absDrift < 500) return 'Soft Sync (Correcting)';
    return 'Drift Warning (Hard seeking)';
  };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {videos.map((vid) => (
                <button
                  ...
                  className={`flex flex-col gap-1 p-4 rounded-xl border text-left transition-all relative overflow-hidden ${
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg flex flex-col gap-5">
            ...
            <div className="bg-slate-950/60 border border-slate-800/80 px-4 py-3 rounded-xl flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Sequence ID</div>
                <div className="font-bold text-indigo-400">{session?.sequenceNumber ?? 0}</div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <RotateCcw size={16} /> RESTART VIDEO
              </button>
            </div>
          </div>
            ...
            <div className="h-60 w-full">
              {driftHistory.length === 0 ? (
                <div className="h-full flex items-center justify-center text-slate-600 text-xs">
                  Awaiting display clients telemetry to plot graph...
                </div>
              ) : (
            ...
            <div className="flex flex-col gap-4">
              {displays.length === 0 ? (
                <div className="text-center py-16 border border-dashed border-slate-800 rounded-xl text-slate-600 text-xs">
                  ...
                  <br />
                  <span className="text-[10px] text-slate-700">Open display page in new tabs</span>
                </div>
              ) : (
                displays.map((display) => (
                  <div
                    key={display.clientId}
                    className="bg-slate-950 border border-slate-800/80 rounded-xl p-4 flex flex-col gap-3 transition-all hover:border-slate-700"
                  >
                    {/* Header */}
                    <div className="flex items-center justify-between border-b border-slate-800/60 pb-2">
                      <span className="font-mono text-xs font-bold text-indigo-400">{display.clientId}</span>
                      <div className="flex items-center gap-1.5">
                        <div className={`w-2 h-2 rounded-full ${getStatusColor(display.drift)}`} />
                        <span className="text-[10px] text-slate-400 font-bold uppercase">{getStatusLabel(display.drift)}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                      <div>
                        <span className="text-slate-500 text-[10px] uppercase block">Playback Time</span>
                        <span className="font-mono font-bold text-slate-300">{formatTime(display.currentPosition)}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 text-[10px] uppercase block">Estimated Drift</span>
                      ...
                      <div>
                        <span className="text-slate-500 text-[10px] uppercase block">Network Latency</span>
                        <span className="font-mono font-bold text-slate-300">{display.latency} ms</span>
                      </div>
                      <div>
                        <span className="text-slate-500 text-[10px] uppercase block">Buffer Ahead</span>
                    ...
                    <div className="flex items-center justify-between bg-slate-900 border border-slate-800/60 px-3 py-1.5 rounded-lg text-[10px] font-bold text-slate-400">
                      ...
                      <span>SOCKET: <span className="text-emerald-400">ONLINE</span></span>
                    </div>
                  </div>
                ))
              )}
